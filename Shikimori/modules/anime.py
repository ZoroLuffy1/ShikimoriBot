...
def site_search(update: Update, context: CallbackContext, site: str):
    ...
    if site == "kaizoku":
        ...
        result = f"<b>Search results for</b> <code>{html.escape(search_query)}</code> <b>on</b> @estanimes: \n"
        for entry in search_result:

            if entry.text.strip() == "Nothing Found":
                result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> @estanimes"
                more_results = False
                break
...
EST_SEARCH_HANDLER = DisableAbleCommandHandler("est", est)
